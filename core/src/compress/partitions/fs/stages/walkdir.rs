...
pub fn walkdir_fn(base_root: &Path, target_root: &Path) -> Result<FsDraft> {
    ...
    // Blob SHA-256 cache keyed by abs_path — lets upload_blobs skip re-reading
    // files from NBD just to hash them.  Covers all target regular files.
    let blob_sha256: HashMap<std::path::PathBuf, [u8; 32]> = target_snap
        .values()
        .filter_map(|e| e.sha256.map(|h| (e.abs_path.clone(), h)))
        .collect();
